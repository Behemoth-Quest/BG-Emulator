...
public class RedeemCode implements IRequest
{
   ...
   public void process(String[] params, User user, World world, Room room) throws RequestException
   {
      ...
      QueryResult redeemResults = world.db.jdbc.query("SELECT * FROM redeem_codes WHERE Code = ?", codeInserted);

      if (redeemResults.next()) {
         int codeTimeLeft = world.db.jdbc.queryForInt("SELECT TIMESTAMPDIFF(SECOND, NOW(), ?)", redeemResults.getString("DateExpiry"));
         codeTimeLeft = Math.max(codeTimeLeft, 0);

         if (codeTimeLeft > 0) {
            int userRedeemCount = world.db.jdbc.queryForInt("SELECT COUNT(*) AS rowcount FROM users_redeems WHERE UserID = ? AND RedeemID = ?", user.properties.get("dbId"), redeemResults.getInt("id"));
            if (userRedeemCount < 1) {
               ...
               if (redeemResults.getInt("Gold") > 0) {
                  world.users.giveRewards(user, 0, redeemResults.getInt("Gold"), 0, 0, 0, -1, user.getUserId(), "p");
               }

               if (redeemResults.getInt("Exp") > 0) {
                  world.users.giveRewards(user, redeemResults.getInt("Exp"), 0, 0, 0, 0, -1, user.getUserId(), "p");
               }

               if (redeemResults.getInt("ClassPoints") > 0) {
                  world.users.giveRewards(user, 0, 0, 0, redeemResults.getInt("ClassPoints"), 0, -1, user.getUserId(), "p");
               }
